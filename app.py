...
with tab_alert:
    ...
    if inventory_df is not None and not inventory_df.empty:
        ...
        if cur_col and safe_col:
            ...
            st.info("스프레드시트에 '현재재고' 및 '안전재고' 열이 포함되어 있어야 경고 목록을 출력할 수 있습니다.")
    else:
    ...
    with st.form("inventory_action_form"):
        ...
        if submitted:
            if not item_name:
                ...
                # 폼 제출 로직 (안내)
                st.success(f"[{action_type}] '{item_name}' {qty}개 등록 완료! (구글 시트로 자동 반영 준비 완료)")
                st.info("※ 구글 시트와의 실시간 양방향 자동 쓰기 기능은 Apps Script의 doPost() 함수 설정 후 자동 처리됩니다.")
